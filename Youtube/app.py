...
df= pd.read_excel("D://Code//Youtube//Adidas.xlsx", engine='openpyxl')
st.set_page_config(layout='wide')
# st.markdown('<styles>div.block-continer{padding-top:1rem;}</style>', unsafe_allow_html=True)
# Image.open("D://Code//Youtube//adidass-logo.jpg")
# Image.open("adidass-logo.jpg")
# print(df)
...
with view1:
    ...
    expander.write(data)
with dwn1:
    st.download_button("Get Data", data=data.to_csv().encode("utf-8"),file_name="Retailers.csv",mime="text/csv")
...
# add the units sold as a linechart and a line chart on a secondary y axis
fig3 =go.Figure()
...
with view3:  
    expander=st.expander("View the Data for sales by units sold")
    expander.write(result1)
with dwn3:
    st.download_button("Get Data", data =result.to_csv().encode('utf-8'),
                       file_name="Sale_by_units.csv", mime="text/csv")
st.divider()    
